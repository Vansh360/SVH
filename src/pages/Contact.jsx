...
const DEFAULTS = {
  ...
  email:       "info@sarvodayavk.edu.in",
  address:     "Near Main Bus Stop, Hingna Road",
  city:        "Hingna, Nagpur, Maharashtra – 441110",
  officeHours: "Mon – Sat, 8:00 AM – 2:30 PM",
  facebookUrl:  "https://www.facebook.com/YOUR_ID",
  twitterUrl:   "https://twitter.com/YOUR_ID",
  instagramUrl: "https://www.instagram.com/YOUR_ID",
  whatsappNum:  "91XXXXXXXXXX",
  mapSrc:       "https://www.google.com/maps?q=Hingna,Nagpur,Maharashtra&output=embed",
};

export default function Contact() {
  const [form, setForm] = useState({ name: "", email: "", phone: "", stream: "", message: "" });
  const [status, setStatus] = useState(null);

  /* ── Load contact info saved by admin ── */
  const [info, setInfo] = useState(DEFAULTS);
  useEffect(() => {
    const saved = JSON.parse(localStorage.getItem("contactInfo") || "{}");
    if (Object.keys(saved).length > 0) setInfo({ ...DEFAULTS, ...saved });
  }, []);
    ...
    <>
      ...
      <div className="contact-page">
        ...
        <div className="contact-hero">
          ...
          <p className="hero-sub">Reach out to us for admissions, queries, or general information. We're happy to help.</p>
        </div>
        ...
        <div className="contact-grid">
          ...
          <div className="left-col">
            ...
            <div className="info-card">
              ...
              <div className="info-card-body">
                <strong>{info.phone}</strong><br />
                {info.email}<br /><br />
                <strong>Office Hours:</strong> {info.officeHours}
              </div>
            </div>
            ...
            <div className="social-card">
              ...
              <div className="social-buttons">
                <a href={info.facebookUrl} target="_blank" rel="noopener noreferrer" className="social-btn facebook">
                  <span className="s-icon">f</span> Facebook
                </a>
                <a href={info.twitterUrl} target="_blank" rel="noopener noreferrer" className="social-btn twitter">
                  <span className="s-icon">𝕏</span> Twitter / X
                </a>
                <a href={info.instagramUrl} target="_blank" rel="noopener noreferrer" className="social-btn instagram">
                  <span className="s-icon">◎</span> Instagram
                </a>
              </div>
              <a href={`https://wa.me/${info.whatsappNum}`} target="_blank" rel="noopener noreferrer" className="whatsapp-btn">
                💬 Chat on WhatsApp
              </a>
            </div>
          ...
          <div className="form-card" id="admission">
            <div className="form-header">
              ...
              <p className="form-desc">Fill in the details below and our team will get back to you within 24 hours.</p>
            </div>

            {status === "success" && (
              <div className="form-msg success">✅ Admission form submitted successfully! We'll contact you soon.</div>
            )}
            {status === "error" && (
              <div className="form-msg error">❌ Something went wrong. Please try again or call us directly.</div>
            )}
            ...
            <form onSubmit={handleSubmit}>
              <div className="field-row">
                <div className="field-group" style={{ marginBottom: 0 }}>
                  ...
                  <input className="field-input" type="text" name="name" placeholder="Full name"
                    value={form.name} onChange={handleChange} required />
                </div>
                <div className="field-group" style={{ marginBottom: 0 }}>
                  <label>Phone Number</label>
                  <input className="field-input" type="text" name="phone" placeholder="+91 XXXXX XXXXX"
              ...
              <div className="field-group">
                ...
                <input className="field-input" type="email" name="email" placeholder="your@email.com"
                ...
                <div className="stream-row">
                  {["Science", "Commerce", "Arts", "Not decided"].map((s) => (
                    <div className="stream-option" key={s}>
                      <input type="radio" id={`stream-${s}`} name="stream" value={s}
                        checked={form.stream === s} onChange={handleChange} />
                      <label className="stream-label" htmlFor={`stream-${s}`}>
                        <span className="stream-dot" />{s}
                      </label>
                    </div>
                  ))}
                </div>
              ...
              <div className="field-group">
                ...
                <textarea className="field-input" name="message"
