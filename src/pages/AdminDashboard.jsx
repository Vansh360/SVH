...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    // Load principal info
    const p = JSON.parse(localStorage.getItem("principalInfo") || "{}");
    if (p.name)  setPrincipalName(p.name);
    if (p.qual)  setPrincipalQual(p.qual);
    if (p.exp)   setPrincipalExp(p.exp);
    if (p.msg)   setPrincipalMsg(p.msg);
  }, [navigate]);
  ...
  const savePrincipal = () => {
    ...
    localStorage.setItem("principalInfo", JSON.stringify(data));
    setPrincipalSaved(true);
    setTimeout(() => setPrincipalSaved(false), 3000);
  };
        ...
        <div className="adm-body">
          ...
          <div className="adm-stats">
            {[
              ...
              { icon: "🖼️", label: "Gallery",    num: gallery.length },
          ...
          <div className="adm-grid-3">
            ...
            <Panel icon="📢" title="Notices" count={notices.length}>
              <div className="adm-input-row">
                <input className="adm-input" type="text" placeholder="Enter notice..."
          ...
          {/* ── GALLERY + PRINCIPAL SIDE BY SIDE ── */}
          <div className="adm-grid-2">
            ...
            <Panel icon="🖼️" title="Gallery" count={gallery.length}>
              ...
              <button className="adm-btn-gold" onClick={uploadImage}
                disabled={!imageFile}
            ...
            {/* Principal Info Manager */}
            <Panel icon="👩‍💼" title="Principal Info">
              <div className="adm-field-group">
                <label className="adm-field-label">Full Name</label>
                <input className="adm-input" type="text" placeholder="Dr. Sunita Sharma"
                  value={principalName} onChange={e => setPrincipalName(e.target.value)} />
              </div>
              <div className="adm-field-group">
                <label className="adm-field-label">Qualification</label>
                <input className="adm-input" type="text" placeholder="M.Sc., B.Ed., Ph.D."
                  value={principalQual} onChange={e => setPrincipalQual(e.target.value)} />
              </div>
              <div className="adm-field-group">
                <label className="adm-field-label">Experience</label>
                <input className="adm-input" type="text" placeholder="25+ Years"
                  value={principalExp} onChange={e => setPrincipalExp(e.target.value)} />
              </div>
              <div className="adm-field-group">
                <label className="adm-field-label">Message (short)</label>
                <textarea className="adm-input" placeholder="Write a short message from the principal..."
                  value={principalMsg} onChange={e => setPrincipalMsg(e.target.value)} />
              </div>
              <div className="adm-save-row">
                {principalSaved && (
                  <span className="adm-save-badge">✅ Saved successfully</span>
                )}
                <button className="adm-btn-gold" onClick={savePrincipal} style={{ marginLeft: "10px" }}>
                  💾 Save Changes
                </button>
              </div>
            </Panel>
